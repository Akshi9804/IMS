...
public class OrderService {
    ...
    public CommonResponse<String> addOutgoingOrder(Order order,String userId) {
        ...
        if(item.isPresent() && item.get().getQuantity()>=order.getQuantity())
        {
            ...
            if(order.getQuantity()>locationDetails.get().getStockDetails().get(order.getItemId()))
            {
                return Utility.getResponse(new StatusEntry(ResponseEnum.FAILED), "Quantity not available");
            }
        ...
        userActivityLogService.addUserActivityLog(userId,"Order placed", "Outgoing order of "+item.get().getName()+" of quantity "+order.getQuantity()+" is placed");
